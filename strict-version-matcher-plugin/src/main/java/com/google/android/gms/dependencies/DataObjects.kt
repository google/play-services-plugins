...
data class Dependency(val fromArtifactVersion: ArtifactVersion, val toArtifact: Artifact,
    ...
    init {
        val enableStrictMatching = toArtifact.groupId == "com.google.android.gms" ||
                toArtifact.groupId == "com.google.firebase"
...
class ArtifactDependencyManager {
    ...
    @VisibleForTesting
    internal val dependencies: HashMap<Artifact, ArrayList<Dependency>> = HashMap()

    fun addDependency(dependency: Dependency) {
        synchronized(dependencyLock) {
            var depListForArtifact = dependencies[dependency.toArtifact]
...
data class SemVerInfo(val major: Int, val minor: Int, val patch: Int) {
    companion object {
        fun parseString(versionString: String): SemVerInfo {
            ...
            // TODO: Deal with everything that might exist after the dash.
