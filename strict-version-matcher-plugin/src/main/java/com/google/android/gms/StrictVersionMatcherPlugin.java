...
/**
 * This plugin attaches to the Gradle project dependency resolution process
 * in order to alert when the strict version declarations ("[16.0.0]"
 * dependency declarations in POM files) in Google Play services libraries
 * aren't being maintained.
 * <p>
 * Specifically, when an exact version is specified in Google Play services
 * library POM files it means the libraries were ProGuard/R8'ed together to
 * optimize symbols across the library group (AARs). Those changes aren't
 * represented in the SemVer version declarations and unknown behavior is
 * exhibited when the strict version number declarations are ignored. The
 * strict number declaration can be skipped by Gradle's default behavior
 * when other libraries request higher versions of libraries as part of the
 * dependency resolution process so this plugin breaks the build to warn of
 * the situation and provides paths to the problematic dependency paths.
 *
...
public class StrictVersionMatcherPlugin implements Plugin<Project> {
    ...
    /**
     * Static tracking of dependency information across all modules the plugin
     * is applied to. It is thread-safe because so multi-threaded builds may
     * continue to add references while other project are using the data stored.
     */
    private static DependencyAnalyzer dependencyTracker = new DependencyAnalyzer();

    private static void registerDependenciesGlobally(ResolvableDependencies resolvableDependencies, String projectName, String taskName) {
        ...
        for (DependencyResult depResult : resolutionResult.getAllDependencies()) {
            ...
            dependencyTracker.registerDependency(Dependency.Companion.fromArtifactVersions(fromDep, toDep));
        }
    }
    ...
    private static void printNode(int depth, Node n) {
        ...
        if ("gradle.project".equals(n.getDependency().getFromArtifactVersion().getGroupId())) {
            String fromRef = dep.getFromArtifactVersion().getGradleRef().replace("gradle.project", "");
            String toRef = dep.getToArtifact().getGradleRef().replace("com.google.android.gms", "c.g.a.g").replace("com.google.firebase", "c.g.f");
            logger.warn(prefix.toString() + fromRef + " task/module dep -> " + toRef + "@" + dep.getToArtifactVersionString());
        } else {
            String fromRef = dep.getFromArtifactVersion().getGradleRef().replace("com.google.android.gms", "c.g.a.g").replace("com.google.firebase", "c.g.f");
            String toRef = dep.getToArtifact().getGradleRef().replace("com.google.android.gms", "c.g.a.g").replace("com.google.firebase", "c.g.f");
            logger.warn(prefix.toString() + fromRef + " library depends -> " + toRef + "@" + dep.getToArtifactVersionString());
        }
        if (n.getChild() != null) {
    ...
    private static class DependencyInspector implements DependencyResolutionListener {
        ...
        public void afterResolve(ResolvableDependencies resolvableDependencies) {
            ...
            // Create an Artifact to ArtifactVersion mapping from the Artifacts resolved for the build.
            ...
            Collection<Dependency> activeDeps = dependencyTracker.getActiveDependencies(resolvedVersions.values());
            ...
            for (Dependency dep : activeDeps) {
                ...
                if (!dep.isVersionCompatible(resolvedVersion.getVersion())) {
                    logger.warn("Dependency ended up being resolved to an incompatible version: " + dep);
                    ...
                    GradleException exception = new GradleException("One resolved Google Play services library dependency depends on another at an exact version (e.g. \"[1.4.3]\"), but isn't being resolved to that version. Behavior exhibited by the library will be unknown. Execute gradle from the command line with ./gradlew --info :app:assembleDebug to see the dependency paths to the artifact. Dependency failing: " + dep.getDisplayString() + " but " + dep.getToArtifact().getArtifactId() + " version was " + resolvedVersion.getVersion() + ". This error came from the strict-dep-checker-plugin and can be disabled by disabling that plugin at your own risk. ");
                    // TODO: Warn, not fail, when the Major version boundaries are breached.
                    // TODO: Experiment with collecting all issues and reporting them at once. This will require simplification of the current output.
                    Collection<Node> depsPaths = dependencyTracker.getPaths(resolvedVersion.getArtifact());
