...
class GoogleServicesPlugin implements Plugin<Project> {
    ...
    public final
    static Pattern VARIANT_PATTERN = ~/(?:([^\p{javaUpperCase}]+)((?:\p{javaUpperCase}[^\p{javaUpperCase}]*)*)\/)?([^\/]*)/
    // Some example of things that match this pattern are:
    // "TestTheFlavor"
    // "FlavorsOfTheRainbow"
    // "Test"
    ...
    private static void setupPlugin(Project project, PluginType pluginType) {
        switch (pluginType) {
    ...
    static class GoogleServicesPluginConfig {
